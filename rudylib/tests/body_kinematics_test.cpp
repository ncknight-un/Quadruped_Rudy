...
TEST_CASE("FK-FK consistency - fk produces the same positions each time", "[fk]") {
...
TEST_CASE("FK-IK consistency - IK should return angles that produce the original foot positions when passed through FK", "[fk][ik]") {
    auto body = makeTestBody();

    std::vector<rudylib::JointAngles> original_angles = {
        {0.2, -0.4, -0.8},   // FL
        {0.1, 0.3, -0.5},    // FR
        {-0.2, 0.5, -1.0},   // BL
        {-0.1, -0.3, -0.7}   // BR
    ...
    rudylib::foot_pos target = body.update_FK(original_angles);
    std::vector<rudylib::JointAngles> ik_angles = body.calc_IK(target);
    rudylib::foot_pos reconstructed = body.update_FK(ik_angles);

    REQUIRE_THAT(reconstructed.p_fl.x(), WithinRel(target.p_fl.x(), EPS));
    REQUIRE_THAT(reconstructed.p_fl.y(), WithinRel(target.p_fl.y(), EPS));
    REQUIRE_THAT(reconstructed.p_fl.z(), WithinRel(target.p_fl.z(), EPS));

    // TODO: Fix the Right leg FK/IK issue. At the moment when constructed to a robot as a whole it is failing.
    // REQUIRE_THAT(reconstructed.p_fr.x(), WithinRel(target.p_fr.x(), EPS));
    // REQUIRE_THAT(reconstructed.p_fr.y(), WithinRel(target.p_fr.y(), EPS));
    // REQUIRE_THAT(reconstructed.p_fr.z(), WithinRel(target.p_fr.z(), EPS));
